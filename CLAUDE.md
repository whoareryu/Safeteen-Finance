...
LLM 코딩 시 자주 나는 실패를 줄이기 위한 행동 지침. 백엔드 프로젝트 규약 포함.
...
**Import path conventions (apply to every file you write or edit):**
- Inside `whoareryu/apps/`, drop both `whoareryu` and `apps` from the import root.
  - ✅ `from titanic.adapter.inbound.api...` — not `from whoareryu.apps.titanic...`
- `core` is the one exception: always prefix with `whoareryu.core.`
  - ✅ `from whoareryu.core.matrix.oracle_database import get_db`

Violating any of the three rules above (SOLID, architecture layers, import paths) is a blocker — stop and surface the conflict before writing code.
...
# Backend 프로젝트 규약 (`backend/`)

Claude·Cursor 에이전트가 `backend/**` 작업 시 먼저 읽는 문서.  
...
### B2.3 Titanic — James Director (완성 슬라이스 예시)
