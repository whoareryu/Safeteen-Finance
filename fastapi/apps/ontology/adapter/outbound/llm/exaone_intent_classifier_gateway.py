...
출력 스키마:
{"destination": "exaone_rag" | "gemini" | "crud", "entities": ["핵심 단어"]}

[분류 규칙 — 반드시 순서대로 확인]
1. 질문이 "등록해줘", "추가해줘", "삭제해줘", "수정해줘", "지워줘" 같은 명령형 동사로 데이터 생성·수정·삭제를 직접 요구하면 → "crud"
2. 그 외 식물 종류·품종·증상·병징·관리법·물주기·비료 등 식물 관리 지식이나 사내 전문 도메인 지식을 묻는 질문(있어?, 뭐야?, 왜?, 어떻게?, 알려줘 포함)은 전부 → "exaone_rag"
...
질문: "잎이 노랗게 변했어, 왜 그럴까?"
답변: {"destination": "exaone_rag", "entities": ["잎", "노랗게 변함"]}

질문: "몬스테라 물 주기 주기가 어떻게 돼?"
답변: {"destination": "exaone_rag", "entities": ["몬스테라", "물 주기"]}
...
class ExaoneIntentClassifierGateway(IntentClassificationGateway):
    """PoC 단계 + EXAONE 3.5 2.4B 조건에서는 QLoRA 파인튜닝 없이,
