...
새 DB·백엔드 서비스(컨테이너, compose service 등)를 만들기 전에 아래를 먼저 확인한다.
...
3. **DB 커넥션 문자열 확인**
   - 메인 앱 DB는 도커 컨테이너로 직접 띄우는 pgvector(PostgreSQL) — `fastapi/.env`의 `DATABASE_URL`이 이미 가리키는 컨테이너/DB가 있는지 먼저 확인한다.
   - 새 DB를 새 컨테이너로 만들 것인지, 기존 pgvector 컨테이너에 스키마·테이블만 추가할 것인지는 서로 다른 결정이다 — 구분해서 사용자에게 확인한다.
